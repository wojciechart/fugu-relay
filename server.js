...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/keycheck') {
    const k = process.env.SAKANA_KEY || '';
    return send(res, 200, 'application/json', JSON.stringify({
      keyLen: k.length,
      startsWithBearer: k.startsWith('Bearer'),
      startsWithFish: k.startsWith('fish_'),
      trimmedDiffers: k !== k.trim(),
      relaySecretLen: (process.env.RELAY_SECRET || '').length
    }));
  }
